...
**Secrets:** no plaintext committed. `breakout0-sealedsecret.yaml` holds only ciphertext
(sealed strict-scope for namespace sdk-apps-dev); the controller unseals it into
`Secret breakout0-secrets`, consumed via `envFrom`. Non-secret env (incl. public
`INTERACTIVE_KEY`) lives in `breakout0-config`.

## Environment

| Env | Service | Namespace | Host | Health |
| --- | ------- | --------- | ---- | ------ |
| `dev` | `breakout0` | `sdk-apps-dev` | breakout0-dev-topia.topia-rtsdk.com | `/api/system/health` |
